...
class i_disk_VC: UIViewController {
    ...
    func addSolveButton() {
        ...
        btn.addTarget(self, action: #selector(i_disk_VC.btnAction(_ :)), for: .touchUpInside)
    }
    ...
    func addClearAllButton() {
        ...
        btn2.addTarget(self, action: #selector(i_disk_VC.btnClearAction(_ :)), for: .touchUpInside)
    }
    ...
    @objc func btnAction(_ sender: UIButton!) {
        ...
        answerBox.text = "I(disk) = \(answer)"
        
        setUpBox.text = "I = Mass * Radius^2 / 2 \nI = \(a) * \(b)^2 / 2 \nI = \(answer)"
    }
    ...
    private func configureTapGesture() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(i_disk_VC.handleTap))
...
extension i_disk_VC: UITextFieldDelegate {
