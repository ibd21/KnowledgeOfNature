...
class g_r_VC: UIViewController {
    ...
    func addSolveButton() {
        ...
        btn.addTarget(self, action: #selector(g_r_VC.btnAction(_ :)), for: .touchUpInside)
    }
    ...
    func addClearAllButton() {
        ...
        btn2.addTarget(self, action: #selector(g_r_VC.btnClearAction(_ :)), for: .touchUpInside)
    }
    ...
    @objc func btnAction(_ sender: UIButton!) {
        ...
        c = Double(radiusTF.text!)!
        
        answer = (a * b) / pow(c, 2)
        
        answerBox.text = "g(r) = \(answer)"
        setUpBox.text = "g(r) = ( Gravity Constant * Mass of Planet ) / Radius^2\ng(r) = ( \(a) * \(b) ) / \(c)^2 \ng(r) = \(answer)"
    ...
    private func configureTapGesture() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(g_r_VC.handleTap))
...
extension g_r_VC: UITextFieldDelegate {
